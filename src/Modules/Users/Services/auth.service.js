...
export const SignUpService = async (req, res) => {
  ...
  if (isFullNameExists) return res.status(409).json({ message: "Full name already exists" });
...
export const ConfirmEmailService = async (req, res) => {
  ...
  if (!user) return res.status(400).json({ message: "User not found or already exists" });
...
export const ForgetPasswordService = async (req, res) => {
  try {
    ...
    if (!user.otps) user.otps = {};
    user.otps.resetPassword = { code: hashSync(otp, +process.env.SALT_ROUNDS), expiresAt: otpExpired };
    await user.save();
...
export const ResetPasswordService = async (req, res) => {
  ...
  const user = await User.findOne({ email, provider: ProviderEnum.LOCAL });
  if (!user) return res.status(404).json({ message: "User not found" });
  if (!user.otps?.resetPassword?.code) return res.status(400).json({ message: "OTP not found" });
  if (Date.now() > user.otps.resetPassword.expiresAt) return res.status(400).json({ message: "OTP expired" });
...
export const UpdatePasswordService = async (req, res) => {
  ...
  res.status(200).json({
    message: "Password updated successfully. Please log in again to continue securely.",
  });
};
...
export const AuthServiceWithGmail = async (req, res) => {
  ...
  let user = await User.findOne({ googleSub: sub, provider: ProviderEnum.GOOGLE });
  if (!user) {
