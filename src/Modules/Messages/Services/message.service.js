...
export const getMessagesService = async (req, res) => {
  ...
  const message = await Messages.find({ receiverId: _id }).populate([
    {
      path: "receiverId",
...
export const getAllPublicMessageService = async (req, res) => {
  const publicMessage = await Messages.find({ isPublic: true }).populate(
    "receiverId",
    "firstName lastName"
  )
  if (!publicMessage) {
  ...
  return res
    .status(201)
    .json({ message: "All public messages fetched successfully", publicMessage })
}
